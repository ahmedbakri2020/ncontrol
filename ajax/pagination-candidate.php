...
if (isset($_POST['filter'])) {
    ...
    $query = $obj->db->query("select * from candidates where $condition limit " . $start . ", " . $perPage . " ");
    $count = $obj->countRow($obj->db->query("select * from candidates where $condition"));
     $displayRecords = $obj->countRow($query);
} elseif (isset($_POST['search'])) {
    $searchid = $obj->StringInputCleaner($_POST['search']);
    $query = $obj->db->query("select * from candidates where name like '$searchid%' limit " . $start . ", " . $perPage . "");
    $count = $obj->countRow($obj->db->query("select * from candidates where name like '%$searchid%' "));
     $displayRecords = $obj->countRow($query);
} else {
    $query = $obj->select('candidates', "*", NULL, array('vote_count'=>'desc'), array($start, $perPage));
    $count = $obj->countRow($obj->select("candidates", "uin"));
     $displayRecords = $obj->countRow($query);
}
